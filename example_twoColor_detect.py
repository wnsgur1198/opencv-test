...
while True:
    ...
    # 색입력을 받기 전이라면
    if not set_color:
        rectangle_color = (0, 255, 0)   # 초록색 사각형 그림 -> 첫번째 색입력을 받을 위치로 사용됨

        if step == 1:
            rectangle_color = (0, 0, 255)   # 빨간색 사각형 그림 -> 두번째 색입력을 위함
    ...
    if key == 27:  # esc
        ...
        # 첫번째 색입력에 대한 처리
        if step == 0:
            # 스페이스바를 누르면 입력영역의 픽셀을 가져옴
            roi = img_color2[cy - 20:cy + 20, cx - 20:cx + 20]
            roi = cv.medianBlur(roi, 3)
            cv.imshow("roi1", roi)
            hsv = cv.cvtColor(roi, cv.COLOR_BGR2HSV)
            h, s, v = cv.split(hsv)
            color1 = h.mean()   # Hue 성분의 평균을 구함
            print(color1)
            step += 1       # Hue 성분의 평균은 첫번째 색 추출에 사용됨
